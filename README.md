<div align="center">

<img src="logo.png" width="128" alt="PriceFinder Logo">

# 🎯 PriceFinder - 慢慢买商品比价插件

🤖 AstrBot 价格搜索插件 — 搜索慢慢买商品比价信息

</div>

---

## 📖 简介

本项目是一个高中生做着玩的**vibe-coding**项目，使用 AI 辅助编程完成开发。（~~感谢雷神免费~~）

---

## 🚀 核心功能

- ✅ **商品比价搜索**: 从慢慢买 (ManManBuy) 抓取商品比价信息，包含标题、价格、商城、链接
- ✅ **智能缓存**: 缓存搜索结果避免重复爬取，支持 TTL 过期自动清理
- ✅ **向量语义搜索**: 即使关键词不完全匹配，也能找到语义相似的缓存结果
- ✅ **AI 结果过滤**: 使用 LLM 对搜索结果去重、排序、过滤，提升结果质量
- ✅ **LLM Tool**: AI 可自主调用 `search_prices` 工具查询商品价格
- ✅ **商品收藏夹**: 收藏感兴趣的商品，支持添加/查看/删除
- ✅ **查询历史**: 查看个人最近50条查询记录
- ✅ **WebUI 仪表盘**: 商品比价 + 收藏夹双 Tab，侧边栏筛选 + 数据表格 + 分页 + 主题切换
- ⬜ **多数据源支持**: 支持喵喵折、购物党、什么值得买等更多第三方平台的比价
- ⬜ **缓存策略优化**: 支持商品的简易分类，实现不同商品不同的过期时间和清理时间配置
- ⬜ **价格历史追踪**: 记录商品价格变化趋势，生成价格走势图
- ⬜ **降价提醒**: 设置目标价格，当商品降价时主动通知用户（~~虽然慢慢买app本身就支持这个功能~~）
- ⬜ **批量搜索**: 支持一次搜索多个关键词
- ⬜ **数据导出**: 将搜索结果导出为 CSV/Excel 格式

---

## 🏁 快速开始

### 📦 安装

将插件文件夹放入 AstrBot 的 `data/plugins` 目录。AstrBot 会自动安装依赖。

### ⚙️ 配置

在 AstrBot WebUI 的插件管理中配置。主要设置项：

**🔍 搜索设置**:
- `max_results`: 最大返回结果数（默认 5）

**💾 缓存设置**:
- `cache_enabled`: 启用缓存（默认 true）
- `cache_ttl_days`: 缓存有效期天数（默认 1）
- `vector_search_enabled`: 启用向量语义搜索（默认 true）

**🤖 AI 过滤设置**:
- `ai_filter_enabled`: 启用 AI 过滤（默认 true）
- `ai_filter_provider`: LLM 提供商 ID（留空使用 AstrBot 默认）

**🔧 调试设置**:
- `debug_enabled`: 启用详细日志输出（默认 false）

**👤 用户数据设置**:
- `favorites_enabled`: 启用收藏夹功能（默认 true）

---

## 💻 命令

| 命令 | 说明 |
| :--- | :--- |
| `/price` / `/price help` | 📖 查看完整中文使用指南 |
| `/price search <关键词>` | 🔍 搜索慢慢买商品比价信息 |
| `/price history` | 📋 查看我的查询历史（最近50条） |
| `/price fav add <序号>` | ⭐ 收藏上次搜索结果的第N条 |
| `/price fav list` / `我的收藏` | 📋 查看我的收藏列表 |
| `/price fav remove <序号>` / `取消收藏 <序号>` | ❌ 删除指定收藏 |

---

## 🏗️ 架构

### 🧩 核心组件

1. **💾 CacheManager**
   - JSON 文件缓存 + 向量相似度搜索
   - TTL 过期自动清理 + LRU 淘汰
   - 全局查询统计计数器

2. **🕷️ ManManBuy Scraper**
   - 使用 httpx 抓取 `s.manmanbuy.com` 搜索结果
   - BeautifulSoup 解析 CSS 选择器提取商品信息

3. **🧠 AI Filter**
   - 调用 LLM 剔除无关结果 + 提取品牌/商品名/型号
   - 过滤结果写入缓存，后续请求零 LLM 消耗
   - 降级策略：无 LLM 提供商时使用原始结果

4. **🖥️ WebUI Dashboard**
   - 符合 AstrBot 插件 Pages 规范的仪表盘
   - 商品比价 + 收藏夹双 Tab，Header 一键切换
   - 侧边栏筛选 + 数据表格 + 分页 + 主题切换
   - 支持缓存条目删除、悬停完整显示、调试模式

---

## 📜 许可证

AGPL-3.0
